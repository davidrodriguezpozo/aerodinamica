...
% which indicates the name of the mesh
    ...
    conv = zeros(datos.Nx,datos.Ny);
    diff = zeros(datos.Nx,datos.Ny);
    ...
    for i = 2:datos.Nx-1
        ...
        if strcmp(malla,'x') == 1
            ...
            conv(i,j) = Sup*(u*du_dx + u*dv_dx); %u * du/dx + u * dv/dx
            diff(i,j) = Sup*(d2u_dx2); 
            
        elseif strcmp(malla,'y') == 1
            
            conv(i,j) = Sup*(v*du_dy + v*dv_dy); %v * du/dy + v * dv/dy
            diff(i,j) = Sup*(d2v_dy2); 
